...
static pthread_mutex_t  locks[HASH_TABLE_CAPACITY] = {[0 ... (HASH_TABLE_CAPACITY-1)] = PTHREAD_MUTEX_INITIALIZER};
                             ...
                             uint32_t value)
{
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	uint32_t index = bernstein_hash(key) % HASH_TABLE_CAPACITY;
	int l = pthread_mutex_lock(&(locks[index]));
	if (l!=0){
	...
	if (list_entry != NULL) {
		int u = pthread_mutex_unlock(&(locks[index]));
		if (u!=0){
			// hash_table_v2_destroy(hash_table);
			exit(u);
		}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int d = pthread_mutex_destroy(&(locks[i]));
		if (d!=0){
			free(hash_table);
			exit(d);
		}
